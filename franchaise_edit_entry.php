...
if (isset($_GET['id'])) {
    $entry_id = $_GET['id'];
    if (preg_match('/\d+/', $entry_id, $matches)) {
        $number = $matches[0]; // Extracted number (944 in this case)
        //echo ($number);
    } else {
        echo "No valid number found in the ID.";
        exit;
    }

    // Query to get the data for the given ID
    $query = "SELECT * FROM franchaise_booking WHERE entry_no = $number";
...
<body>
    <div class="container mt-1">
        ...
        <P2 class="fw-bold">Download the policy copy from the below link</P2>
            <table class="table mt-1 ">
                ...
                <td><a href="<?php echo str_replace('.htm', '.pdf', $data['file_path']); ?>" target="_blank">&#8595;</a></td>
                <td><?php echo $data['customer_name']; ?></td>  
            </tbody>
            </table>
        </div>

        <form method="POST" action="franchaise_update_entry.php?id=<?php echo $number; ?>">
                ...
                <div class="row border border-success rounded bg-success bg-gradient bg-opacity-10 shadow-sm mx-1 ">
                    ...
                    <div class="row"><!--- Start of row ----->
                        ...
                        <div class="row"><!--- Start of row ----->
                            ...
                            <div class="col-sm-2 mb-3">
                                <!-- Outward Point -->
                                <label for="fincommission" class="form-label">Franchaise In Point</label>
                                <input type="text" class="form-control" id="fincommission" name="fincommission"
                                    value="<?php echo htmlspecialchars($data['franchaise_inward_point']); ?>" required>
                            </div>
                            <div class="col-sm-2 mb-3">
                                <!-- Outward Point -->
                                <label for="foutcommission" class="form-label">Franchaise Out Point</label>
                                <input type="text" class="form-control" id="foutcommission" name="foutcommission"
                                    value="<?php echo htmlspecialchars($data['franchaise_outward_point']); ?>" required>
                            </div>
                            <div class="col-sm-2 mb-3">
                                <!-- Margin -->
                                <label for="margin" class="form-label">Our Margin</label>
                                <input type="text" class="form-control" id="margin" name="margin"
                                    value="<?php echo htmlspecialchars($data['margin']); ?>" required>
                            </div>
                            <div class="col-sm-2 mb-3">
                                <!-- franchaise Margin -->
                                <label for="margin" class="form-label">Franchaise Margin</label>
                                <input type="text" class="form-control" id="fmargin" name="fmargin"
                                    value="<?php echo htmlspecialchars($data['franchaise_margin']); ?>" required>
                            </div>
                        </div>
                    </div><!--- End of row ----->
                    <hr class="my-3 border-success">
    ...
    <script>
        function toggleMarginInput() {
            ...
            var odoutward = document.getElementById("fodinward").value;
            var tpoutward = document.getElementById("ftpinward").value;
            var netoutward = document.getElementById("fnetinward").value;

            var faodoutward = document.getElementById("fodoutward").value;
            var ftpoutward = document.getElementById("ftpoutward").value;
            var fnetoutward = document.getElementById("fnetoutward").value;
            ...
            var franchaiseincommission = document.getElementById("fincommission").value;
            ...
            if (odinward != 0 && tpinward != 0) {
                ...
                //document.getElementById("franchaiseincommission").value = franchaiseincommission.toFixed(2);
                var margin = inwardcommission - franchaiseincommission;
