...
if (isset($_POST['submit'])) {
    if (isset($_POST['month'])) {
        ...
        $query = "SELECT 
                    ...
                    f_year,                   
                    sm_remarks 
                    FROM franchaise_booking WHERE sm_name='$managername' AND month_name='$entrymonthlabel' AND f_year='$entryfyearlabel'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // Set the CSV headers
            $headers = [
                'Entry Date', 'Entry No', 'Customer Name', 'Vehicle No','RTO Code', 'Product Type', 'Sub Product', 'Segment', 
                'Plan', 'Insurance Co', 'Policy No',  
                 'Policy Start Date', 'Policy End Date',
                'Engine Number', 'Chassi Number', 'Make', 'Model', 
                'Total Premium', 'Net Premium','Commissionable Premium', 
                'Payment Mode', 'POS Code', 'POS Name', 'Non POS Name',
                'SM Name',
                'OD Inward', 'TP Inward', 'Net Inward', 
                'OD Outward', 'TP Outward', 'Net Outward', 
                'Commission In Before TDS','Commission Out Before TDS','Status',
                'Business Month', 'Business Year', 'Remarks' 
