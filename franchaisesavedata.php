...
if (isset($_POST['submit'])) {
    ...
    $tpinward = "0";
    $netinward = "0";
    $fodinward = strip_tags($_POST['odinward']);
    $ftpinward = strip_tags($_POST['tpinward']);
    $fnetinward = strip_tags($_POST['netinward']);
    $fodoutward = strip_tags($_POST['odoutward']);
    $ftpoutward = strip_tags($_POST['tpoutward']);
    $fnetoutward = strip_tags($_POST['netoutward']);
    $remarks = ""; // initilization for branch remarks
    $llpremium = strip_tags($_POST['llpremium']);
    $monthname = strip_tags($_POST['monthname']);
    $oinwardpoint = "0"; // Initialize
    $inwardpoint = strip_tags($_POST['incommission']);
    $outwardpoint = strip_tags($_POST['outcommission']);
    $margin = strip_tags($_POST['margin']);
    $omargin = "0"; // Initialize
    ...
    $insertQuery = "INSERT INTO franchaise_booking (
        entry_date,
        state_name,
        product_type,
        sub_product,
    ...
    //$update_sql = "UPDATE franchaise_booking SET our_entry_no = '$entryno_prefix' WHERE entry_no = (SELECT MAX(entry_no) FROM daily_booking)";
    $update_sql = "UPDATE franchaise_booking SET our_entry_no = '$entryno_prefix' WHERE entry_no = '$entryno'";
    $result = mysqli_query($con, $update_sql);
    //echo ($entryno_prefix);
    $_SESSION['last_entry_no'] = $entryno_prefix;

    // Redirect to businessentry.php
    header("Location: franchaisebusinessentry.php");
    exit();
}
?>