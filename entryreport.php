...
<body>
    ...
    <div class="cont my-1 ">
        ...
        <div class="row">
            ...
            <form action="reportdownload.php" method="post" enctype="multipart/form-data">
                    <div class="row border border-primary mx-3 bg-primary bg-gradient bg-opacity-10 shadow-sm p-3">
                        <div class="col-sm-2 m-1 p-2 mx-5">
                            <select id="entrydate" name="entrydate" class="form-select" aria-label="Default select example">
                                ...
                                <?php
                                $zonalid = "SELECT mapped_zone_id FROM `newemp` where `name`='$_SESSION[username]'";
                                $zipresult = mysqli_query($con, $posname) or die(mysqli_error());
                                $zirow = mysqli_fetch_array($zipresult);
                                if ($zirow > 0) {
                                    $mzid = $zirow['mapped_zone_id'];
                                }
                                
                                $query = "SELECT distinct entry_date FROM `daily_booking` where mapped_zone_id = '$mzid'";
