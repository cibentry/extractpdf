...
if (isset($_POST['submit'])) {
    if (isset($_POST['fromdate']) && isset($_POST['todate']))  {
        ...
        $formatted_from_date = date('d-m-Y', strtotime($fromdate));
        $formatted_to_date = date('d-m-Y', strtotime($todate));
    ...
    sm_remarks
FROM daily_booking 
WHERE STR_TO_DATE(entry_date, '%d-%m-%Y') 
        BETWEEN STR_TO_DATE('$formatted_from_date', '%d-%m-%Y') 
        AND STR_TO_DATE('$formatted_to_date', '%d-%m-%Y') 
    ";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // Write the text in the first row
            fputcsv($output, ["Showing report for the period: $formatted_from_date to $formatted_to_date"]);
            ...
            $headers = [
                ...
                'Location Name', 'SM Name','Employee Code','OD Inward', 'TP Inward', 'Net Inward', 
                ...
                'Mapped Zone', 'Mapped Zone ID','Employee Type', 'File Path', 'SM Remarks'
            ];
